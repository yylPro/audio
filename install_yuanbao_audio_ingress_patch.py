...
def patch_package(package: Path) -> None:
    ...
    patched_line = ".use(audioQualityIngress) // Deterministic audio-quality ingress; bypasses Agent"
    if patched_line not in updated:
        dispatch_marker = ".use(dispatchReply)"
        if dispatch_marker not in updated:
            raise RuntimeError(f"找不到管线插入锚点：{create_file}")
        updated = updated.replace(dispatch_marker, patched_line + "\n        " + dispatch_marker, 1)
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    if updated != original:
        shutil.copy2(create_file, create_file.with_name(f"create.js.before-audio-ingress-{timestamp}.bak"))
